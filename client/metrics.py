...
def generate_graphs(csv_path: str) -> None:
    ...
    # Segmentos onde ocorreu failover (mudança de servidor)
    ...
    for i, seg in enumerate(failover_segments):
        ax1.axvline(x=seg, color='purple', linestyle='--', linewidth=1.5,
                    label='Failover' if i == 0 else "")

    plt.title('Entrega 2 - Vazão vs Qualidade')
    ...
    for i, seg in enumerate(rebuffer_segments):
        plt.axvline(x=seg, color='red', linestyle=':', label='Rebuffering' if i == 0 else "")
    ...
    plt.title('Entrega 2 - Evolução do Buffer')
    plt.legend()
    plt.grid(True, linestyle='--')
